...
const PaymentStatusTypeForm = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (validate()) {
      ...
      console.log(value);
      if(StatusType === null){
        axios.post(`/paymentstatustype`,value).then(
          (res) => {
        ...
        axios.put(`/paymentstatustype/${value.id}`,value).then(
          (res) => {
  ...
  const titleName = () => {
    if(StatusType == null) 
      return "Ajouter un statut de remboursement" 
    else
      return "Modifier un statut de remboursement"
  }
...
export default PaymentStatusTypeForm;
