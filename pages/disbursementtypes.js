...
import DisbursementTypeForm from "./add_disbursementtype";
...
export default function EnhancedTable() {
  ...
  React.useEffect(() => {
      ...
      /* apiService.getCurrentMonth().then(
        ...
        //if(cMonth){
          apiService.getDisbursementsTypes( //pageNumber, pageSize, getBy
            ).then(
              res => {
  ...
  const push = (e) =>{
    //DisbursementTypes.push(e)
    apiService.getDisbursementsTypes( //pageNumber, pageSize, getBy, currentMonth.monthId
      ).then(
        res => {
          console.log(res.data);
          setDisbursementTypes(res.data);
  ...
  const update = (e) =>{
    var objIndex = DisbursementTypes.findIndex((obj => obj.id == e.id));
    DisbursementTypes[objIndex] = e
  }

  const remove = () =>{
    if(selected !== null){
      apiService.deleteDisbursementType(selected.id).then(
        res => {
          console.log(res);
          const index = DisbursementTypes.indexOf(selected);
          DisbursementTypes.splice(index, 1);
          setDelete(!deleted)
  ...
  return (<>
    {/* {authenticated &&} */}
    {/* {!detail ? */}
    <BaseCard titleColor={"secondary"} title="TYPES DE DECAISSEMENT">
      ...
      <Dialog fullWidth={true} maxWidth={'lg'} open={open} onClose={handleClose}>
                ...
                <DisbursementTypeForm
                    DisbursementType={selected}
                    push={push}
                    update={update}
    ...
    {/* :
    <DetailDisbursementType clientsCount = {200 } numbersCount = {200 } DisbursementType = {selected} />
    } */}
  </>);
}
