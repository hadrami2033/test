...
import SpendingTypeForm from "./add_spendingtype";
...
export default function EnhancedTable() {
  ...
  const [SpendingTypes, setSpendingTypes] = React.useState([])
  ...
  React.useEffect(() => {
      ...
      /* apiService.getCurrentMonth().then(
        ...
        //if(cMonth){
          apiService.getSpendingsTypes( //pageNumber, pageSize, getBy
            ).then(
              res => {
                console.log(res.data);
                setSpendingTypes(res.data);
  ...
  const push = (e) =>{
    //SpendingTypes.push(e)
    apiService.getSpendingsTypes( //pageNumber, pageSize, getBy, currentMonth.monthId
      ).then(
        res => {
          console.log(res.data);
          setSpendingTypes(res.data);
  ...
  const update = (e) =>{
    var objIndex = SpendingTypes.findIndex((obj => obj.id == e.id));
    SpendingTypes[objIndex] = e
  }

  const remove = () =>{
    if(selected !== null){
      apiService.deleteSpendingType(selected.id).then(
        res => {
          console.log(res);
          const index = SpendingTypes.indexOf(selected);
          SpendingTypes.splice(index, 1);
          setDelete(!deleted)
  ...
  return (<>
    {/* {authenticated &&} */}
    {/* {!detail ? */}
    <BaseCard titleColor={"secondary"} title="TYPE DE DEPENSE">
