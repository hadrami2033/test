...
import BorrowerForm from "./add_borrower";
...
export default function EnhancedTable() {
  ...
  React.useEffect(() => {
      ...
      /* apiService.getCurrentMonth().then(
        ...
        //if(cMonth){
          apiService.getBorrowers( //pageNumber, pageSize, getBy
            ).then(
              res => {
  ...
  const push = (e) =>{
    //Borrowers.push(e)
    apiService.getBorrowers( //pageNumber, pageSize, getBy, currentMonth.monthId
      ).then(
        res => {
          console.log(res.data);
          setBorrowers(res.data);
  ...
  const update = (e) =>{
    var objIndex = Borrowers.findIndex((obj => obj.id == e.id));
    Borrowers[objIndex] = e
  }

  const remove = () =>{
    if(selected !== null){
      apiService.deleteBorrower(selected.id).then(
        res => {
          console.log(res);
          const index = Borrowers.indexOf(selected);
  ...
  return (<>
    {/* {authenticated &&} */}
    {/* {!detail ? */}
    <BaseCard titleColor={"secondary"} title="GESTION DES EMPRUNTEURS">
    ...
    {/* :
    <DetailBorrower clientsCount = {200 } numbersCount = {200 } Borrower = {selected} />
    } */}
  </>);
}
